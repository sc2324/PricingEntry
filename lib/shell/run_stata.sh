...
run_stata() {
    ...
    files_before=$(find "$OUTPUT_DIR" -type f ! -name "make.log" -exec basename -a {} + | tr '\n' ' ')
   ...
   # parse the log file to check for Stata error codes like `r(error)`
    ...
    files_after=$(find "$OUTPUT_DIR" -type f -newermt "$start_time" ! -name "make.log" -exec basename -a {} + | tr '\n' ' ')
